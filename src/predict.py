...
class FootballScorePredictor:
    ...
    def _get_team_features(self, team: str, is_home: bool, opponent: str) -> Dict:
        ...
        if matches:
            ...
            features["momentum"] = sum(
                1 if r == "W" else -1 if r == "L" else 0 for r in results[-5:]
            )
        else:
            features["gf_ewm"] = 1.4
            features["ga_ewm"] = 1.2
        ...
        features["attack_rating"] = (
            features["gf_ewm"] / 1.4 if len(matches) >= 5 else 1.0
        )
        features["defense_rating"] = (
            1.2 / max(features["ga_ewm"], 0.5) if len(matches) >= 5 else 1.0
        )
    ...
    def build_features(self, home_team: str, away_team: str) -> pd.DataFrame:
        ...
        home_feats = self._get_team_features(
            home_team, is_home=True, opponent=away_team
        )
        away_feats = self._get_team_features(
            away_team, is_home=False, opponent=home_team
        )
    ...
    def predict_lambda(self, home_team: str, away_team: str) -> Tuple[float, float]:
        ...
        scale = 1.8
        lambda_home = (
            self.global_home_avg + (lambda_home - self.global_home_avg) * scale
        )
        lambda_away = (
            self.global_away_avg + (lambda_away - self.global_away_avg) * scale
        )
    ...
    def predict(self, home_team: str, away_team: str) -> Dict:
        ...
        prob_matrix, lambda_home, lambda_away = self.predict_score_probabilities(
            home_team, away_team
        )
        ...
        predicted_home, predicted_away, confidence = (
            scores[0] if scores else flat_probs[0]
        )
    ...
    def predict_batch(
        self, fixtures: List[Tuple[str, str]], verbose: bool = True
    ) -> List[Dict]:
...
if __name__ == "__main__":
    ...
    for result in predictor.predict_batch(fixtures):
        print(
            f"{result['home_team']} vs {result['away_team']}: {result['predicted_score']} ({result['confidence']:.1%})"
        )
        print(
            f"  λ={result['lambda_home']:.2f}/{result['lambda_away']:.2f}  "
