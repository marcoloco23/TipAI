...
def load_football_data_co_uk():
    ...
    for filepath in RAW_DATA_DIR.glob("*.csv"):
        ...
        try:
            ...
            match_df = match_df.dropna(subset=['Date', 'HomeTeam', 'AwayTeam', 'HomeGoals', 'AwayGoals'])
...
def load_champions_league_data():
    ...
    if cl_current.exists():
        try:
            ...
            if 'Home Team' in df.columns and 'Away Team' in df.columns:
                ...
                match_df = match_df.dropna(subset=['Date', 'HomeTeam', 'AwayTeam', 'HomeGoals', 'AwayGoals'])
                all_matches.append(match_df)
                print(f"  Loaded champions_league_2024.csv: {len(match_df)} completed matches")
    ...
    for filepath in RAW_DATA_DIR.glob("cl_*.csv"):
        try:
            ...
            home_col = next((c for c in df.columns if 'home' in c.lower() or c == 'Team 1'), None)
            away_col = next((c for c in df.columns if 'away' in c.lower() or c == 'Team 2'), None)
            score_col = next((c for c in df.columns if 'score' in c.lower() or c == 'FT'), None)
            ...
            if score_col:
                ...
                hg_col = next((c for c in df.columns if c in ['HG', 'FTHG', 'Home Goals']), None)
                ag_col = next((c for c in df.columns if c in ['AG', 'FTAG', 'Away Goals']), None)
            ...
            match_df = match_df.dropna(subset=['HomeTeam', 'AwayTeam', 'HomeGoals', 'AwayGoals'])
            all_matches.append(match_df)
