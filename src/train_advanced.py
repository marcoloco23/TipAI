...
class AdvancedFeatureBuilder:
    ...
    def get_team_features(self, team, is_home, opponent):
        ...
        for window in [3, 5, 10]:
            ...
            venue_recent = venue_matches[-window:] if len(venue_matches) >= window else venue_matches
        ...
        if matches:
            ...
            momentum = sum(1 if r == 'W' else (-1 if r == 'L' else 0) for r in recent_results)
    ...
    def update_history(self, row):
        ...
        self.team_matches[home].append({
            ...
            'result': home_result, 'points': home_pts
        })

        self.team_matches[away].append({
            'date': date, 'opponent': home, 'is_home': False,
            'goals_for': ag, 'goals_against': hg,
            'result': away_result, 'points': away_pts
        })

        # Update h2h
        h2h_key = tuple(sorted([home, away]))
        self.h2h_matches[h2h_key].append({
            'date': date, 'team': home, 'goals_for': hg, 'goals_against': ag, 'result': home_result
        })
        self.h2h_matches[h2h_key].append({
            'date': date, 'team': away, 'goals_for': ag, 'goals_against': hg, 'result': away_result
        })
...
def train_models(feature_df, n_tune_trials=30):
    ...
    for model_type in ['xgb', 'lgb', 'cat']:
        ...
        best_params = tune_model(model_type, X_train, y_home_train, X_val, y_home_val, n_tune_trials)
    ...
    for model_type in ['xgb', 'lgb', 'cat']:
        ...
        best_params = tune_model(model_type, X_train, y_away_train, X_val, y_away_val, n_tune_trials)
    ...
    meta_val_home = np.column_stack([predictions['val'][f'{m}_home'] for m in ['xgb', 'lgb', 'cat']])
    meta_test_home = np.column_stack([predictions['test'][f'{m}_home'] for m in ['xgb', 'lgb', 'cat']])
    meta_val_away = np.column_stack([predictions['val'][f'{m}_away'] for m in ['xgb', 'lgb', 'cat']])
    meta_test_away = np.column_stack([predictions['test'][f'{m}_away'] for m in ['xgb', 'lgb', 'cat']])
    ...
    for model_type in ['xgb', 'lgb', 'cat']:
        ...
        exact = np.sum((np.round(pred_home) == y_home_test) & (np.round(pred_away) == y_away_test))
        exact_pct = exact / len(y_home_test) * 100

        print(f"\n{model_type.upper()}: MAE home={mae_home:.3f}, away={mae_away:.3f}, Exact={exact_pct:.1f}%")

    # Ensemble (simple average)
    ensemble_home = np.mean([predictions['test'][f'{m}_home'] for m in ['xgb', 'lgb', 'cat']], axis=0)
    ensemble_away = np.mean([predictions['test'][f'{m}_away'] for m in ['xgb', 'lgb', 'cat']], axis=0)

    mae_home = mean_absolute_error(y_home_test, ensemble_home)
    mae_away = mean_absolute_error(y_away_test, ensemble_away)
    exact = np.sum((np.round(ensemble_home) == y_home_test) & (np.round(ensemble_away) == y_away_test))
    exact_pct = exact / len(y_home_test) * 100
    ...
    exact = np.sum((np.round(stacked_home) == y_home_test) & (np.round(stacked_away) == y_away_test))
    exact_pct = exact / len(y_home_test) * 100
