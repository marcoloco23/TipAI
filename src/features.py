...
def calculate_rolling_stats(df, team_col, goals_for_col, goals_against_col, window=5, prefix=''):
    """Calculate rolling statistics for a team."""
    stats = {}

    # Goals
    stats[f'{prefix}goals_scored_avg_{window}'] = df[goals_for_col].rolling(window, min_periods=1).mean()
    stats[f'{prefix}goals_conceded_avg_{window}'] = df[goals_against_col].rolling(window, min_periods=1).mean()

    # Clean sheets
    stats[f'{prefix}clean_sheets_{window}'] = (df[goals_against_col] == 0).rolling(window, min_periods=1).sum()
...
def build_team_stats(matches_df):
    ...
    for idx, row in tqdm(matches_df.iterrows(), total=len(matches_df), desc="Building features"):
        ...
        if len(home_history) >= 3:
            ...
            features['home_goals_conceded_home_avg5'] = recent['AwayGoals'].tail(5).mean()
            features['home_goals_scored_home_avg10'] = recent['HomeGoals'].mean()
            features['home_goals_conceded_home_avg10'] = recent['AwayGoals'].mean()
            features['home_home_wins_last5'] = (recent['HomeGoals'].tail(5) > recent['AwayGoals'].tail(5)).sum()
        else:
        ...
        if len(away_history) >= 3:
            ...
            features['away_goals_conceded_away_avg5'] = recent['HomeGoals'].tail(5).mean()
            features['away_goals_scored_away_avg10'] = recent['AwayGoals'].mean()
            features['away_goals_conceded_away_avg10'] = recent['HomeGoals'].mean()
            features['away_away_wins_last5'] = (recent['AwayGoals'].tail(5) > recent['HomeGoals'].tail(5)).sum()
        else:
        ...
        if len(h2h_matches) >= 2:
            ...
            home_h2h_goals = h2h_df.apply(
                lambda x: x['goals_for'] if x.get('team') == home_team else x['goals_against'],
                axis=1
            )
            away_h2h_goals = h2h_df.apply(
                lambda x: x['goals_for'] if x.get('team') == away_team else x['goals_against'],
                axis=1
            )
